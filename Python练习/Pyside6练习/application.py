# 创建一个画板
from PySide6.QtWidgets import QHBoxLayout, QVBoxLayout, QPushButton, QSplitter, QWidget,\
    QComboBox, QLabel, QSpinBox, QFileDialog,QCheckBox,QApplication
from PySide6.QtGui import QPixmap, QPainter, QPaintEvent, QMouseEvent, QPen, \
    QColor, QIcon
...
class MainWidget(QWidget):
    ...
    def __InitView(self):
        ...
        self.setWindowTitle("PaintBoard Example PySide6")
...
class PaintBoard(QWidget):
    ...
    def ChangePenThickness(self, thickness=10):  # 改变画笔粗细
        self.__thickness = thickness

    def IsEmpty(self):  # 返回画板是否为空
        return self.__IsEmpty

    def GetContentAsQImage(self):  # 获取画板内容（返回QImage）
    ...
    def mousePressEvent(self, mouseEvent):  # 鼠标按下时，获取鼠标的当前位置保存为上一次位置
        self.__currentPos = mouseEvent.pos()
        self.__lastPos = self.__currentPos

    def mouseMoveEvent(self, mouseEvent):  # 鼠标移动时，更新当前位置，并在上一个位置和当前位置间画线
        self.__currentPos = mouseEvent.pos()
        self.__painter.begin(self.__board)
        if self.EraserMode == False:  # 非橡皮擦模式
            self.__painter.setPen(QPen(self.__penColor, self.__thickness))  # 设置画笔颜色，粗细
        else:
            self.__painter.setPen(QPen(Qt.white, 10))  # 橡皮擦模式下画笔为纯白色，粗细为10

        self.__painter.drawLine(self.__lastPos, self.__currentPos)  # 画线
        self.__painter.end()
        self.__lastPos = self.__currentPos
